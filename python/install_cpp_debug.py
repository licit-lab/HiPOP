...
def build_hipop_cpp(prefix):
    ...
    try:
        subprocess.check_call(fr"cmake -S '{src_dir}' -DCMAKE_PREFIX_PATH={prefix} -DCMAKE_INSTALL_PREFIX={prefix} -DCMAKE_BUILD_TYPE=Debug", shell=True)
        subprocess.check_call(r"cmake --build . --target install --config Debug", shell=True)
    except:
        print("Build failed ..")
        sys.exit(-1)
    finally:
